...
class _Dark {
  ...
  // Sparkline painter backgrounds
  static const chartBg   = Color(0xFF0B1220);
...
// ─── Stock Data Model ─────────────────────────────────────────────────────────
class _Stock {
  final String ticker, name, last, bid, ask, change;
  ...
  Color color(bool isLight) => isFlat
      ? (isLight ? _Light.grey : _Dark.grey)
      : (isUp
      ? (isLight ? _Light.teal : _Dark.teal)
      : (isLight ? _Light.red  : _Dark.red));
  ...
  factory _Stock.fromJson(Map<String, dynamic> json) {
    ...
    String fmt(double v) => v == 0 ? '—' : 'E ${v.toStringAsFixed(2)}';

    String changeLabel;
    if (changePct == 0 && changeVal == 0) {
      changeLabel = '0.0%';
    } else if (changePct != 0) {
      changeLabel = '${changePct > 0 ? '+' : ''}${changePct.toStringAsFixed(1)}%';
    } else {
      changeLabel = '${changeVal > 0 ? '+' : ''}E ${changeVal.toStringAsFixed(2)}';
    }
  ...
  static String _deriveTicker(String code, String fullname) {
    ...
    if (letters.length >= 2 && letters.length <= 6) return letters.toUpperCase();
    final words = fullname.trim().split(RegExp(r'\s+'));
    if (words.length == 1) {
      return words.first.substring(0, math.min(5, words.first.length)).toUpperCase();
    }
    return words.take(5).map((w) => w.isNotEmpty ? w[0].toUpperCase() : '').join();
  }
  ...
  static List<double> _buildSparkline({
    ...
    final allPrices = [open, close, high, low, last].where((p) => p > 0).toList();
    ...
    double norm(double v) => range == 0 ? 0.5 : 1.0 - ((v - minP) / range).clamp(0.0, 1.0);
...
class MarketWatch extends StatefulWidget {
  ...
  const MarketWatch({super.key, required this.themeNotifier});
...
class _MarketWatchState extends State<MarketWatch>
  ...
  Future<void> _fetchMarketData() async {
    ...
    try {
      ...
      if (mounted) setState(() { _errorMessage = 'Network error: ${e.message}'; _loading = false; });
    } catch (_) {
      if (mounted) setState(() { _errorMessage = 'Unable to load market data.'; _loading = false; });
    }
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Row(children: [
                ...
                _LiveBadge(teal: _teal, surface: _surface, border: _border),
              ]),
            ],
          ),
          ...
          if (_loading)
            _LoadingState(card: _card, surface: _surface, border: _border)
            ...
            ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                if (i >= _cardAnims.length) return card;
...
class _LoadingState extends StatelessWidget {
  ...
  const _LoadingState({required this.card, required this.surface, required this.border});

  @override
  Widget build(BuildContext context) {
    return Column(
        children: List.generate(4, (_) =>
            _SkeletonCard(card: card, surface: surface, border: border)));
  }
}

class _SkeletonCard extends StatefulWidget {
  final Color card, surface, border;
  const _SkeletonCard({required this.card, required this.surface, required this.border});
      ...
      builder: (_, __) => Container(
        ...
        decoration: BoxDecoration(
          ...
          gradient: LinearGradient(
            colors: [widget.card, widget.surface, widget.card],
            stops: [
...
class _ErrorState extends StatelessWidget {
  ...
  final Color card, border, red, gold, textSub;
  final bool isLight;

  const _ErrorState({
    required this.message, required this.onRetry,
    required this.card, required this.border, required this.red,
    required this.gold, required this.textSub, required this.isLight,
  });
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(children: [
        Icon(Icons.wifi_off_rounded, color: red.withOpacity(0.70), size: 32),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Text('Retry', style: TextStyle(fontSize: 12.5,
...
class _LiveBadge extends StatefulWidget {
  final Color teal, surface, border;
  const _LiveBadge({required this.teal, required this.surface, required this.border});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: widget.teal.withOpacity(0.30), width: 1),
      ),
        ...
        AnimatedBuilder(
          ...
          builder: (_, __) => Container(
            ...
            decoration: BoxDecoration(
              ...
              color: widget.teal.withOpacity(0.4 + 0.6 * _pulse.value),
            ),
          ),
        ),
        const SizedBox(width: 5),
        Text('Market Open', style: TextStyle(fontSize: 9.5,
            fontWeight: FontWeight.w700, color: widget.teal)),
      ]),
    );
  }
}
...
class _StockCard extends StatefulWidget {
  final _Stock stock;
  final bool   isLight;
  final Color  teal, red, cardBg, textPrim, textSub;

  const _StockCard({
    required this.stock, required this.isLight,
    required this.teal, required this.red,
    required this.cardBg, required this.textPrim, required this.textSub,
  });
...
class _StockCardState extends State<_StockCard>
  ...
  Widget build(BuildContext context) {
    ...
    final c = s.color(widget.isLight);
    final chartBg   = widget.isLight ? _Light.chartBg   : _Dark.chartBg;
    final chartGrid = widget.isLight ? _Light.chartGrid  : _Dark.chartGrid;
      ...
      child: AnimatedScale(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                  color: c.withOpacity(widget.isLight ? 0.08 : 0.10),
                  blurRadius: 18, offset: const Offset(0, 4)),
              BoxShadow(
                  color: Colors.black.withOpacity(widget.isLight ? 0.06 : 0.28),
                ...
                children: [
                  ...
                  Row(children: [
                    _MicroLabel(label: 'B', value: s.bid,
                        color: widget.teal, textSub: widget.textSub),
                    const SizedBox(width: 10),
                    _MicroLabel(label: 'A', value: s.ask,
                        color: widget.red, textSub: widget.textSub),
                  ]),
                ],
              ),
            ),
...
class _MicroLabel extends StatelessWidget {
  ...
  final Color  color, textSub;
  const _MicroLabel({required this.label, required this.value,
    required this.color, required this.textSub});
  ...
  Widget build(BuildContext context) {
    return Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text(value, style: TextStyle(fontSize: 8.5,
...
class _SparklinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(pts.last, 6.5, Paint()..color = color.withOpacity(0.15));
    canvas.drawCircle(pts.last, 3.2, Paint()..color = color);
    canvas.drawCircle(pts.last, 1.4, Paint()..color = Colors.white.withOpacity(0.90));
  }

  @override
  bool shouldRepaint(_SparklinePainter old) =>
      old.progress != progress || old.color != color ||
          old.chartBg  != chartBg  || old.chartGrid != chartGrid;
}